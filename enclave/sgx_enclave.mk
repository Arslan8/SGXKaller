...
$(SGX_SDK_SOURCE_LIBDIR)/libsgx_trts_sim.a: | $(SGX_SDK_SOURCE_DIR)
	...
	export CC=gcc
	export CXX=g++
	export CC=gcc && export CXX=g++ && $(SGX_SDK_MAKE) -C $(SGX_SDK_SOURCE_DIR)/sdk simulation DEBUG=1
	export CC=$(OLDCC)
	export CXX=$(OLDCXX)
$(SGX_SDK_SOURCE_LIBDIR)/libsgx_%.a: | $(SGX_SDK_SOURCE_DIR)
	...
	export CC=gcc
	export CXX=g++
	export CC=gcc && export CXX=g++ &&  $(SGX_SDK_MAKE) -C $(SGX_SDK_SOURCE_DIR)/sdk $* DEBUG=1
	export CC=$(OLDCC)
	export CXX=$(OLDCXX)
$(SGX_SDK_SOURCE_DIR)/sdk/selib/linux/libselib.a: | $(SGX_SDK_SOURCE_DIR)
	...
	export CC=gcc
	export CXX=g++
	export CC=gcc && export CXX=g++ &&  $(SGX_SDK_MAKE) -C $(SGX_SDK_SOURCE_DIR)/sdk selib DEBUG=1
	export CC=$(OLDCC)
	export CXX=$(OLDCXX)
...
$(SGX_SDK_SOURCE_LIBDIR)/libsgx_urts_sim.so: | $(SGX_SDK_SOURCE_DIR)
	...
	export CC=gcc
	export CXX=g++
	export CC=gcc && export CXX=g++ &&  $(SGX_SDK_MAKE) -C $(SGX_SDK_SOURCE_DIR)/psw simulation DEBUG=1
	export CC=$(OLDCC)
	export CXX=$(OLDCXX)
$(SGX_SDK_SOURCE_LIBDIR)/libsgx_%.so: | $(SGX_SDK_SOURCE_DIR)
	...
	export CC=gcc
	export CXX=g++
	export CC=gcc && export CXX=g++ &&  $(SGX_SDK_MAKE) -C $(SGX_SDK_SOURCE_DIR)/psw $* DEBUG=1
	export CC=$(OLDCC)
	export CXX=$(OLDCXX)
...
$(SGX_SDK_SOURCE_LIBDIR)/sgx_sign: | $(SGX_SDK_SOURCE_DIR)
	...
	export CC=gcc
	export CXX=g++
	export CC=gcc && export CXX=g++ && $(SGX_SDK_MAKE) -C $(SGX_SDK_SOURCE_DIR)/sdk signtool DEBUG=1 	
	export CC=$(OLDCC)
	export CXX=$(OLDCXX)
...
$(SGX_SDK_SOURCE_LIBDIR)/sgx_edger8r: | $(SGX_SDK_SOURCE_DIR)
	...
	export CC=gcc
	export CXX=g++
	export CC=gcc && export CXX=g++ && $(SGX_SDK_MAKE) -C $(SGX_SDK_SOURCE_DIR)/sdk edger8r
	export CC=$(OLDCC)
	export CXX=$(OLDCXX)
...
linux-sgx-$(SGX_SDK_SOURCE_GIT_REV): linux-sgx-$(SGX_SDK_SOURCE_GIT_TAG).git
	cp -r ../linux-sgx-sgx_2.1.3-g75dd558bdaff/ .
...
%_t.c: %.edl %_t.h | $(SGX_EDGER8R)
	mv $*_t.h $*_t.h.bak
	 $(SGX_EDGER8R) --trusted --trusted-dir $(dir $@) --search-path $(SGX_INCLUDEDIR) --search-path $(LIB_SFUZZ) --search-path $(includedir) $<; RES=$$?; mv $*_t.h.bak $*_t.h; exit $$RES
%_t.h: %.edl | $(SGX_EDGER8R)
	 $(SGX_EDGER8R) --trusted --trusted-dir $(dir $@) --search-path $(SGX_INCLUDEDIR) --search-path $(LIB_SFUZZ) --search-path $(includedir) --header-only $<

%_u.c: %.edl %_u.h | $(SGX_EDGER8R)
	mv $*_u.h $*_u.h.bak
	$(SGX_EDGER8R) --untrusted --untrusted-dir $(dir $@) --search-path $(SGX_INCLUDEDIR) --search-path $(LIB_SFUZZ) --search-path $(includedir) $<; RES=$$?; mv $*_u.h.bak $*_u.h; exit $$RES
%_u.h: %.edl | $(SGX_EDGER8R)
	 $(SGX_EDGER8R) --untrusted --untrusted-dir $(dir $@) --search-path $(SGX_INCLUDEDIR) --search-path $(LIB_SFUZZ) --search-path $(includedir) --header-only $<
...
ENCLAVE_CFLAGS = -fvisibility=hidden -fpie -I$(SGX_INCLUDEDIR)/tlibc -O0 -g -I$(LIB_SFUZZ)/Include -fsanitize=address 

ENCLAVE_LDFLAGS = -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L$(libdir) -L$(LIB_SFUZZ) \
	-Wl,--whole-archive -l$(SGX_TRTS_LIB) -lsgx_tsgxfuzz -Wl,--no-whole-archive \
	-Wl,--start-group -lsgx_tstdc -lsgx_tcrypto -lselib -Wl,--end-group \
	-Wl,-Bstatic -lasan -lm -Wl,-Bsymbolic -Wl,--no-allow-shlib-undefined \
...
lib%.unstripped.so: $(includedir)/%_t.o $(libdir)/lib$(SGX_TRTS_LIB).a $(libdir)/libselib.a $(libdir)/libsgx_tstdc.a
	$(CC) -L/home/arslan/sgxsdk2/sgxsdk/SampleCode/pristine/ContactDiscoveryService/enclave/./linux-sgx-sgx_2.1.3-g75dd558bdaff/build/linux/ $(LDFLAGS) -o $@ $(filter %.o,$^) $(LDLIBS) \
